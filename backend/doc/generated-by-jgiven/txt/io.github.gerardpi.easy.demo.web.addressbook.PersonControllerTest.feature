 ...
 Post person
   ...
   When an HTTP "PATCH" on "/api/persons/00000000-1111-2222-3333-444444444444" is performed with body 
          ----------------------------------------------------------------------
          {
                    "name": {
                              "last": "very last"
   ...
   Then the HTTP status code is "400 BAD_REQUEST"
   When an HTTP "PATCH" on "/api/persons/00000000-1111-2222-3333-444444444444" is performed with body 
          ----------------------------------------------------------------------
          {
                    "name": {
 ...
 and headers "{If-Match=0}"
   ...
   Then the response contains body equals 
    ...
    And the response contains header "ETag" with value ""1""
    And the response contains header "Last-Modified"
    And the response contains header "Cache-Control" with value "max-age=60"
    And the response contains header "Content-Type" with value "application/json;charset=UTF-8"
