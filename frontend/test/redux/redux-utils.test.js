...
describe('redux-utils', () => {
  it('creating an action type works as expected', () => {
    expect(reduxUtils.backendAction.command.createType('ABC')).to.be.equal('ABC_CMD');
    expect(reduxUtils.backendAction.ok.createType('ABC')).to.be.equal('ABC_CMD_OK');
    expect(reduxUtils.backendAction.fail.createType('ABC')).to.be.equal('ABC_CMD_FAIL');
  });
