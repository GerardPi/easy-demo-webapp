...
describe('addressbook delete epic', () => {
  ...
  it('address.delete success', (done) => {
    ...
    actualActionArray$.subscribe((actualActionArray) => {
      ...
      expect(actualAction2.type).to.be.equal(addressbookActions.address.readList.command.type);
  ...
  it('address.delete problem', (done) => {
    ...
    const state$ = rxjs.of({});
    // When
    const rejected = new Promise((resolve, reject) => reject(new Error('Something went wrong')));
