...
describe('backendServices', () => {
  ...
  it('toQueryArguments', () => {
    ...
    expect(sut.toQueryArguments({ page: 0, size: 100 })).to.be.equal(
      'page=0&size=100'
    );
    expect(sut.toQueryArguments({ page: 1, size: 100 })).to.be.equal(
      'page=1&size=100'
    );
    expect(sut.toQueryArguments({ page: null, size: 100 })).to.be.equal(
      'size=100'
    );
  });
});
