...
export function toCommandType(okOrFailType) {
  if (okOrFailType.endsWith(ACTION_SUFFIX.fail)) {
    return (
      okOrFailType.substr(0, okOrFailType.length - ACTION_SUFFIX.fail.length) +
      ACTION_SUFFIX.command
    );
  }
  if (okOrFailType.endsWith(ACTION_SUFFIX.ok)) {
    return (
      okOrFailType.substr(0, okOrFailType.length - ACTION_SUFFIX.ok.length) +
      ACTION_SUFFIX.command
    );
  }
  throw new Error(
    `Command type to process must end with either '${ACTION_SUFFIX.fail}' or '${ACTION_SUFFIX.ok}'`
  );
}
...
export const backendAction = {
  command: {
    ...
    isType: actionType =>
      commonUtils.endsWith(actionType, ACTION_SUFFIX.command),
    suffix: ACTION_SUFFIX.command,
  },
  commandRepeat: {
    createType: prefix => prefix + ACTION_SUFFIX.commandRepeat,
    is: action =>
      commonUtils.endsWith(action.type, ACTION_SUFFIX.commandRepeat),
    isType: actionType =>
      commonUtils.endsWith(actionType, ACTION_SUFFIX.commandRepeat),
...
function getUserFeedbackData(commandAction) {
  return commonUtils.isSubvalueNullOrEmpty(
    commandAction.payload,
    'userFeedbackData'
  )
...
export function createCommonSuccessAction(commandAction, someResponse) {
  ...
  const response = commonUtils.isNullOrEmpty(someResponse)
    ? RESPONSE_DEFAULT
    : someResponse;
...
export function createCommonFailureAction(commandAction, someError) {
  const response = commonUtils.isSubvalueNullOrEmpty(someError, 'response')
    ? RESPONSE_DEFAULT
    : someError.response;
