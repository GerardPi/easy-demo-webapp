...
export const contentTypeOptions = {
  ...
  formData: {
    headers: { headerNameContentType: 'application/x-www-form-urlencoded' },
  },
...
export const createBackend = restApi => ({
  ...
  performPostWithJsonBody: (url, jsonBody) =>
    restApi
      .post(url, jsonBody, contentTypeOptions.json.headers)
      .then(response => response.data),
  performPostWithFormData: (url, body) =>
    restApi
      .post(url, body, contentTypeOptions.formData)
      .then(response => response.data),
  performPutWithJsonBody: (url, jsonBody, etag) =>
    restApi
      .put(url, jsonBody, contentTypeOptions.json)
      .then(response => response.data),
  performUpload: (url, uploadFile) => {
    const formData = new FormData();
    formData.append('multipartFile', uploadFile);
    return restApi
      .post(url, formData, contentTypeOptions.upload)
      .then(response => response.data);
  },
