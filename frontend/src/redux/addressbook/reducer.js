...
const reducer = reduxToolkit.createReducer(INITIAL_STATE, {
  ...
  [addressbookActions.address.readList.ok.type]: (state, action) => {
    ...
    state.address.list.selectionData.pageIndex =
      action.payload.response.pageable.page;
    state.address.list.selectionData.pageSize =
      action.payload.response.pageable.size;
  },
  ...
  [addressbookActions.person.readList.ok.type]: (state, action) => {
    ...
    state.person.list.selectionData.pageIndex =
      action.payload.response.pageable.page;
    state.person.list.selectionData.pageSize =
      action.payload.response.pageable.size;
  },
});
