...
function createReadAddressListAction(state) {
  ...
  return addressbookActions.address.readList.command(
    selectionData.pageIndex,
    selectionData.pageSize
  );
}

export const createEpics = backendSvc => ({
  readAddress: (action$, state$) =>
    action$.pipe(
      ofType(addressbookActions.address.read.command.type),
      mergeMap(action =>
        from(
          addressbookServices.address.read(
            backendSvc,
            action.payload.id,
          ...
          map(response =>
            addressbookActions.address.read.ok(
              response,
  ...
  readAddressList: (action$, state$) =>
    action$.pipe(
      ofType(addressbookActions.address.readList.command.type),
      mergeMap(action =>
        from(
          addressbookServices.address.readList(backendSvc, {
            page: action.payload.pageIndex,
            size: action.payload.pageSize,
          })
        ).pipe(
          map(response =>
            addressbookActions.address.readList.ok(
              response,
              reduxUtils.createSuccessMetaData(action)
            )
          ),
  ...
  createAddress: (action$, state$) =>
    action$.pipe(
      ofType(addressbookActions.address.create.command.type),
      mergeMap(action =>
        from(
          addressbookServices.address.create(backendSvc, action.payload.data)
        ).pipe(
          mergeMap(response =>
            from([
              reduxUtils.createCommonSuccessAction(action, response),
              createReadAddressListAction(state$.value),
            ])
          ),
          catchError(error =>
            of(reduxUtils.createCommonFailureAction(action, error))
  ...
  updateAddress: (action$, state$) =>
    action$.pipe(
      ofType(addressbookActions.address.update.command.type),
      mergeMap(action =>
        from(
          addressbookServices.address.update(
            backendSvc,
            action.payload.id,
            action.payload.etag,
            action.payload.data
  ...
  deleteAddress: (action$, state$) =>
    action$.pipe(
      ofType(addressbookActions.address.delete.command.type),
      mergeMap(action =>
        from(
          addressbookServices.address.delete(
            backendSvc,
            action.payload.id,
            action.payload.etag
          )
