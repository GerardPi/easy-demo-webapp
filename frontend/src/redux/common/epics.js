...
const TIME_IN_MS_DISPLAY_TRANSIENT_USER_FEEDBACK = 10000;
...
const epics = {
  ...
  delayedTransientUserFeedbackDeletion: action$ => action$.pipe(
    ofType(commonActions.transientUserFeedback.deleteLater.type),
    delay(TIME_IN_MS_DISPLAY_TRANSIENT_USER_FEEDBACK),
    map(action => commonActions.transientUserFeedback.deleteNow(action.payload.transientUserFeedbackIds)))
};
